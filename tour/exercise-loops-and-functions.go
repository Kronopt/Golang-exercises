...
// Starting with some guess z, we can adjust z based on how close
// z² is to x, producing a better guess:
//
// z -= (z*z - x) / (2*z)
...
// guess for z is 1, no matter what the input. To begin with,
// repeat the calculation 10 times and print each z along the way.
// See how close you get to the answer for various values of
...
// like x, or x/2. How close are your function's results to the math.Sqrt
...
func Sqrt(x float64) float64 {
	...
	for i := 1; i < 50; i++ {
		...
		z -= (z*z - x) / (2 * z)
		if z == z_before {
			fmt.Printf("Reached value in %v iterations\n", i)
			break
		}
		fmt.Println(z)
	}
...
func main() {
	x := 999998.0
	fmt.Println(Sqrt(x), "(Sqrt)")
	fmt.Println(math.Sqrt(x), "(math.Sqrt)")
}

// For some even numbers the result stabilizes in two alternating values
// where one of them is equivalent to math.Sqrt
